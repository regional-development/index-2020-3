...
LAND = ("18010500", "18010600", "18010700", "18010800", "18010900")
ADMIN_REGIONS = {
...
def collect_incomes():
    ...
    # земля
    land = (
        df_current.loc[df_current["INCO"].isin(LAND)]
        .groupby("ADMIN", as_index=False)["EXECUTED"]
        .sum()
    ...
    # ПДФО
    pdfo = (
        df_current.loc[df_current["INCO"].eq("11010000")]
        .groupby("ADMIN", as_index=False)["EXECUTED"]
        .sum()
    ...
    mask = df_previous["FIN_SOURCE"].eq("C") & df_previous["INCO"].str.contains(
        "^[1235]0000000"
    )
    wo_transfers_previous = (
        df_previous.loc[mask]
        .groupby("ADMIN", as_index=False)["EXECUTED"]
        .sum()
